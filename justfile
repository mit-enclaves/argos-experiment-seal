...
build-musl-no-tyche:
  make -C tyche-musl/ -j `nproc` CFLAGS="-static -D RUN_WITHOUT_TYCHE -g3 -ggdb3 -O0 -Wl,-z,norelro"
  make -C tyche-musl/ install
...
setup-seal:
  cmake -S SEAL -B {{SEAL_BUILD}} -DCMAKE_TOOLCHAIN_FILE="{{CUR_DIR}}/seal-toolchain.cmake" -DCMAKE_INSTALL_PREFIX={{ROOT}} -DSEAL_USE_INTRIN=OFF -DSEAL_BUILD_EXAMPLES=ON -DHAVE_THREAD_SAFETY_ATTRIBUTES=OFF -DHAVE_STD_REGEX=ON
